...
namespace HurricaneVR.Framework.Core.Grabbers
{
    public class HVRHandGrabber : HVRGrabberBase
    {
        ...
        public HVRbool3 ClimbingAxis { get; private set; }
        ...
        {
            if (!IsHovering || !_grabIndicator || !HoverTarget.ShowGrabIndicator || !ThisHandCanGrab)
                return;
        ...
        {
            ThisHandCanGrab = true;
            ThisHandCanGrab = grabbable.HandGrabType switch
            {
                HandGrabType.Both => true,
                HandGrabType.Left => IsLeftHand,
                HandGrabType.Right => IsRightHand,
            };
            if (!ThisHandCanGrab) return;
        ...
        {
            if (IsForceGrabbing || !ThisHandCanGrab || (IsGripGrabActive && (!HoverTarget || HoverTarget != grabbable)))
                return false;
        ...
        public override bool CanGrab(HVRGrabbable grabbable)
        {
            ...
            switch (grabbable.HandGrabType)
            {
                case HandGrabType.Both:
                    return true;
                case HandGrabType.Left:
                    return IsLeftHand;
                case HandGrabType.Right:
                    return IsRightHand;
            }
        ...
        private void CommonGrab(HVRGrabbable grabbable)
        {
            ...
            if (grabbable.TryGetComponent(out HVRClimbable climb))
            {
                IsClimbing = true;
                ClimbingAxis = climb.axisToClimb;
            }
